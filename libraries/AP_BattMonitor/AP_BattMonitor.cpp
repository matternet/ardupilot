...
AP_BattMonitor::BatteryFailsafe AP_BattMonitor::check_failsafe(const uint8_t instance)
{
    ...
    if ((voltage_used > 0) && (_params[instance]._critical_voltage > 0) &&
        (voltage_used < _params[instance]._critical_voltage) &&
        (voltage_used > _params[instance]._low_voltage*0.25f)) {
    ...
    if ((voltage_used > 0) && (_params[instance]._low_voltage > 0) &&
        (voltage_used < _params[instance]._low_voltage) &&
        (voltage_used > _params[instance]._low_voltage*0.25f)) {
        // this is the first time our voltage has dropped below minimum so start timer
