...
void NavEKF2_core::readBaroData()
{
    ...
    // do not accept data at a faster rate than 14Hz to avoid overflowing the FIFO buffer
    if (frontend->_baro.get_last_update(selected_baro) - lastBaroReceived_ms > 70) {
        frontend->logging.log_baro = true;

        baroDataNew.hgt = frontend->_baro.get_altitude(selected_baro);
        ...
        lastBaroReceived_ms = frontend->_baro.get_last_update(selected_baro);
