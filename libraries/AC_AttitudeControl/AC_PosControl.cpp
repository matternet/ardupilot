...
#define POSCONTROL_ACC_Z_P                                                     \
  0.3f // vertical acceleration controller P gain default
#define POSCONTROL_ACC_Z_I                                                     \
  1.0f // vertical acceleration controller I gain default
#define POSCONTROL_ACC_Z_D                                                     \
  0.0f // vertical acceleration controller D gain default
#define POSCONTROL_ACC_Z_IMAX                                                  \
  800 // vertical acceleration controller IMAX gain default
#define POSCONTROL_ACC_Z_FILT_HZ                                               \
  10.0f // vertical acceleration controller input filter default
#define POSCONTROL_ACC_Z_DT 0.02f // vertical acceleration controller dt default
#define POSCONTROL_POS_XY_P                                                    \
  1.0f // horizontal position controller P gain default
#define POSCONTROL_VEL_XY_P                                                    \
  1.4f // horizontal velocity controller P gain default
#define POSCONTROL_VEL_XY_I                                                    \
  0.7f // horizontal velocity controller I gain default
#define POSCONTROL_VEL_XY_D                                                    \
  0.35f // horizontal velocity controller D gain default
#define POSCONTROL_VEL_XY_IMAX                                                 \
  1000.0f // horizontal velocity controller IMAX gain default
#define POSCONTROL_VEL_XY_FILT_HZ                                              \
  5.0f // horizontal velocity controller input filter
...
#define POSCONTROL_ACC_Z_I                                                     \
  0.1f // vertical acceleration controller I gain default
#define POSCONTROL_ACC_Z_D                                                     \
  0.0f // vertical acceleration controller D gain default
#define POSCONTROL_ACC_Z_IMAX                                                  \
  100 // vertical acceleration controller IMAX gain default
#define POSCONTROL_ACC_Z_FILT_HZ                                               \
  20.0f // vertical acceleration controller input filter default
#define POSCONTROL_ACC_Z_DT                                                    \
  0.0025f // vertical acceleration controller dt default
#define POSCONTROL_POS_XY_P                                                    \
  1.0f // horizontal position controller P gain default
#define POSCONTROL_VEL_XY_P                                                    \
  1.0f // horizontal velocity controller P gain default
#define POSCONTROL_VEL_XY_I                                                    \
  0.5f // horizontal velocity controller I gain default
#define POSCONTROL_VEL_XY_D                                                    \
  0.0f // horizontal velocity controller D gain default
#define POSCONTROL_VEL_XY_IMAX                                                 \
  1000.0f // horizontal velocity controller IMAX gain default
#define POSCONTROL_VEL_XY_FILT_HZ                                              \
  5.0f // horizontal velocity controller input filter
...
#define POSCONTROL_POS_XY_P                                                    \
  1.0f // horizontal position controller P gain default
#define POSCONTROL_VEL_XY_P                                                    \
  2.0f // horizontal velocity controller P gain default
#define POSCONTROL_VEL_XY_I                                                    \
  1.0f // horizontal velocity controller I gain default
#define POSCONTROL_VEL_XY_D                                                    \
  0.5f // horizontal velocity controller D gain default
#define POSCONTROL_VEL_XY_IMAX                                                 \
  1000.0f // horizontal velocity controller IMAX gain default
#define POSCONTROL_VEL_XY_FILT_HZ                                              \
  5.0f // horizontal velocity controller input filter
...
const AP_Param::GroupInfo AC_PosControl::var_info[] = {
    ...
    // @Description: Lower values will slow the response of the navigation
    // controller and reduce twitchiness
    // @Units: Hz
    // @Range: 0.5 5
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("_ACC_XY_FILT", 1, AC_PosControl, _accel_xy_filt_hz,
                 POSCONTROL_ACCEL_FILTER_HZ),

    // @Param: _POSZ_P
    // @DisplayName: Position (vertical) controller P gain
    // @Description: Position (vertical) controller P gain.  Converts the
    // difference between the desired altitude and actual altitude into a climb
    // or descent rate which is passed to the throttle rate controller
    // @Range: 1.000 3.000
    // @User: Standard
    ...
    // @Description: Velocity (vertical) controller P gain.  Converts the
    // difference between desired vertical speed and actual speed into a desired
    // acceleration that is passed to the throttle acceleration controller
    // @Range: 1.000 8.000
    // @User: Standard
    ...
    // @Description: Acceleration (vertical) controller P gain.  Converts the
    // difference between desired vertical acceleration and actual acceleration
    // into a motor output
    // @Range: 0.500 1.500
    // @Increment: 0.05
    // @User: Standard
    ...
    // @Description: Acceleration (vertical) controller I gain.  Corrects
    // long-term difference in desired vertical acceleration and actual
    // acceleration
    // @Range: 0.000 3.000
    // @User: Standard
    ...
    // @Description: Acceleration (vertical) controller I gain maximum.
    // Constrains the maximum pwm that the I term will generate
    // @Range: 0 1000
    // @Units: d%
    // @User: Standard
    ...
    // @Description: Acceleration (vertical) controller D gain.  Compensates for
    // short-term change in desired vertical acceleration vs actual acceleration
    // @Range: 0.000 0.400
    // @User: Standard
    ...
    // @Param: _ACCZ_FLTD
    // @DisplayName: Acceleration (vertical) controller derivative frequency in
    // Hz
    // @Description: Acceleration (vertical) controller derivative frequency in
    // Hz
    // @Range: 1 100
    // @Increment: 1
    // @Units: Hz
    ...
    // @Description: Position controller P gain.  Converts the distance (in the
    // latitude direction) to the target location into a desired speed which is
    // then passed to the loiter latitude rate controller
    // @Range: 0.500 2.000
    // @User: Standard
    ...
    // @Description: Velocity (horizontal) P gain.  Converts the difference
    // between desired velocity to a target acceleration
    // @Range: 0.1 6.0
    // @Increment: 0.1
    // @User: Advanced
    ...
    // @Description: Velocity (horizontal) I gain.  Corrects long-term
    // difference in desired velocity to a target acceleration
    // @Range: 0.02 1.00
    // @Increment: 0.01
    // @User: Advanced
    ...
    // @Description: Velocity (horizontal) D gain.  Corrects short-term changes
    // in velocity
    // @Range: 0.00 1.00
    // @Increment: 0.001
    // @User: Advanced
    ...
    // @Description: Velocity (horizontal) integrator maximum.  Constrains the
    // target acceleration that the I gain will output
    // @Range: 0 4500
    // @Increment: 10
    // @Units: cm/s/s
    // @User: Advanced
    ...
    // @Description: Velocity (horizontal) input filter.  This filter (in hz) is
    // applied to the input for P and I terms
    // @Range: 0 100
    // @Units: Hz
    // @User: Advanced
    ...
    // @Description: Maximum lean angle autopilot can request.  Set to zero to
    // use ANGLE_MAX parameter value
    // @Units: deg
    // @Range: 0 45
    // @Increment: 1
    // @User: Advanced
...
// their values.
//
AC_PosControl::AC_PosControl(const AP_AHRS_View &ahrs,
                             const AP_InertialNav &inav,
                             const AP_Motors &motors,
                             AC_AttitudeControl &attitude_control)
    : _ahrs(ahrs), _inav(inav), _motors(motors),
      _attitude_control(attitude_control), _p_pos_z(POSCONTROL_POS_Z_P),
      _p_vel_z(POSCONTROL_VEL_Z_P),
      _pid_accel_z(POSCONTROL_ACC_Z_P, POSCONTROL_ACC_Z_I, POSCONTROL_ACC_Z_D,
                   0.0f, POSCONTROL_ACC_Z_IMAX, 0.0f, POSCONTROL_ACC_Z_FILT_HZ,
                   0.0f, POSCONTROL_ACC_Z_DT),
      _p_pos_xy(POSCONTROL_POS_XY_P),
      _pid_vel_xy(POSCONTROL_VEL_XY_P, POSCONTROL_VEL_XY_I, POSCONTROL_VEL_XY_D,
                  POSCONTROL_VEL_XY_IMAX, POSCONTROL_VEL_XY_FILT_HZ,
                  POSCONTROL_VEL_XY_FILT_D_HZ, POSCONTROL_DT_50HZ),
      _dt(POSCONTROL_DT_400HZ), _speed_down_cms(POSCONTROL_SPEED_DOWN),
      _speed_up_cms(POSCONTROL_SPEED_UP), _speed_cms(POSCONTROL_SPEED),
      _accel_z_cms(POSCONTROL_ACCEL_Z), _accel_cms(POSCONTROL_ACCEL_XY),
...
/// set_dt - sets time delta in seconds for all controllers (i.e. 100hz = 0.01,
/// 400hz = 0.0025)
...
void AC_PosControl::set_max_speed_z(float speed_down, float speed_up) {
  ...
  if (is_equal(_speed_down_cms, speed_down) &&
      is_equal(_speed_up_cms, speed_up)) {
    return;
  }
...
///     should be called continuously (with dt set to be the expected time
///     between calls) actual position target will be moved no faster than the
///     speed_down and speed_up target will also be stopped if the motors hit
///     their limits or leash length is exceeded
void AC_PosControl::set_alt_target_with_slew(float alt_cm, float dt) {
  ...
  if ((alt_change < 0 && !_motors.limit.throttle_lower) ||
      (alt_change > 0 && !_motors.limit.throttle_upper)) {
    if (!is_zero(dt)) {
      float climb_rate_cms =
          constrain_float(alt_change / dt, _speed_down_cms, _speed_up_cms);
  ...
  _pos_target.z = constrain_float(_pos_target.z, curr_alt - _leash_down_z,
                                  curr_alt + _leash_up_z);
}

/// set_alt_target_from_climb_rate - adjusts target up or down using a climb
/// rate in cm/s
///     should be called continuously (with dt set to be the expected time
///     between calls) actual position target will be moved no faster than the
///     speed_down and speed_up target will also be stopped if the motors hit
///     their limits or leash length is exceeded
void AC_PosControl::set_alt_target_from_climb_rate(float climb_rate_cms,
                                                   float dt,
                                                   bool force_descend) {
  // adjust desired alt if motors have not hit their limits
  // To-Do: add check of _limit.pos_down?
  if ((climb_rate_cms < 0 &&
       (!_motors.limit.throttle_lower || force_descend)) ||
      (climb_rate_cms > 0 && !_motors.limit.throttle_upper && !_limit.pos_up)) {
    _pos_target.z += climb_rate_cms * dt;
  }
...
/// set_alt_target_from_climb_rate_ff - adjusts target up or down using a climb
/// rate in cm/s using feed-forward
///     should be called continuously (with dt set to be the expected time
///     between calls) actual position target will be moved no faster than the
///     speed_down and speed_up target will also be stopped if the motors hit
///     their limits or leash length is exceeded set force_descend to true
///     during landing to allow target to move low enough to slow the motors
void AC_PosControl::set_alt_target_from_climb_rate_ff(float climb_rate_cms,
                                                      float dt,
                                                      bool force_descend) {
  ...
  if (_vel_desired.z < _speed_down_cms && !is_zero(_speed_down_cms)) {
    accel_z_cms *=
        POSCONTROL_OVERSPEED_GAIN_Z * _vel_desired.z / _speed_down_cms;
  }
  ...
  float accel_z_max =
      MIN(accel_z_cms,
          safe_sqrt(2.0f * fabsf(_vel_desired.z - climb_rate_cms) * jerk_z));
  ...
  _vel_desired.z =
      constrain_float(climb_rate_cms, _vel_desired.z - vel_change_limit,
                      _vel_desired.z + vel_change_limit);
  _flags.use_desvel_ff_z = true;
  ...
  if ((_vel_desired.z < 0 &&
       (!_motors.limit.throttle_lower || force_descend)) ||
      (_vel_desired.z > 0 && !_motors.limit.throttle_upper && !_limit.pos_up)) {
    _pos_target.z += _vel_desired.z * dt;
  }
}

/// add_takeoff_climb_rate - adjusts alt target up or down using a climb rate in
/// cm/s
///     should be called continuously (with dt set to be the expected time
///     between calls) almost no checks are performed on the input
...
void AC_PosControl::relax_alt_hold_controllers(float throttle_setting) {
  ...
  _flags.reset_rate_to_accel_z = true;
  _pid_accel_z.set_integrator(
      (throttle_setting - _motors.get_throttle_hover()) * 1000.0f);
...
/// set_target_to_stopping_point_z - returns reasonable stopping altitude in cm
/// above home
...
void AC_PosControl::init_takeoff() {
  ...
  // shift difference between last motor out and hover throttle into
  // accelerometer I
  _pid_accel_z.set_integrator(
      (_attitude_control.get_throttle_in() - _motors.get_throttle_hover()) *
      1000.0f);
...
// is_active_z - returns true if the z-axis position controller has been run
// very recently
bool AC_PosControl::is_active_z() const {
  return ((AP_HAL::micros64() - _last_update_z_us) <=
          POSCONTROL_ACTIVE_TIMEOUT_US);
}
...
/// calc_leash_length - calculates the vertical leash lengths from maximum
/// speed, acceleration
///     called by update_z_controller if z-axis speed or accelerations are
///     changed
...
// target altitude should be set with one of these functions: set_alt_target,
// set_target_to_stopping_point_z, init_takeoff calculates desired rate in
// earth-frame z axis and passes to rate controller vel_up_max, vel_down_max
// should have already been set before calling this method
void AC_PosControl::run_z_controller() {
  ...
  _vel_target.z = AC_AttitudeControl::sqrt_controller(
      _pos_error.z, _p_pos_z.kP(), _accel_z_cms, _dt);
  ...
  // the following section calculates acceleration required to achieve the
  // velocity target
  ...
  // the following section calculates a desired throttle needed to achieve the
  // acceleration target
  ...
  if (_vibe_comp_enabled) {
    _flags.freeze_ff_z = true;
    _accel_desired.z = 0.0f;
    const float thr_per_accelz_cmss =
        _motors.get_throttle_hover() / (GRAVITY_MSS * 100.0f);
    // during vibration compensation use feed forward with manually calculated
    // gain ToDo: clear pid_info P, I and D terms for logging
    if (!(_motors.limit.throttle_lower || _motors.limit.throttle_upper) ||
        ((is_positive(_pid_accel_z.get_i()) && is_negative(_vel_error.z)) ||
         (is_negative(_pid_accel_z.get_i()) && is_positive(_vel_error.z)))) {
      _pid_accel_z.set_integrator(_pid_accel_z.get_i() +
                                  _dt * thr_per_accelz_cmss * 1000.0f *
                                      _vel_error.z * _p_vel_z.kP() *
                                      POSCONTROL_VIBE_COMP_I_GAIN);
    }
    thr_out =
        POSCONTROL_VIBE_COMP_P_GAIN * thr_per_accelz_cmss * _accel_target.z +
        _pid_accel_z.get_i() * 0.001f;
  } else {
    thr_out = _pid_accel_z.update_all(_accel_target.z, z_accel_meas,
                                      (_motors.limit.throttle_lower ||
                                       _motors.limit.throttle_upper)) *
              0.001f;
  }
  ...
  _attitude_control.set_throttle_out(thr_out, true,
                                     POSCONTROL_THROTTLE_CUTOFF_FREQ);
...
void AC_PosControl::set_pos_target(const Vector3f &position) {
  ...
  _flags.use_desvel_ff_z = false;
  _vel_desired.z = 0.0f;
  // initialise roll and pitch to current roll and pitch.  This avoids a twitch
  // between when the target is set and the pos controller is first run To-Do:
  // this initialisation of roll and pitch targets needs to go somewhere between
  // when pos-control is initialised and when it completes it's first cycle
  //_roll_target =
  // constrain_int32(_ahrs.roll_sensor,-_attitude_control.lean_angle_max(),_attitude_control.lean_angle_max());
  //_pitch_target =
  // constrain_int32(_ahrs.pitch_sensor,-_attitude_control.lean_angle_max(),_attitude_control.lean_angle_max());
}
...
/// set_target_to_stopping_point_xy - sets horizontal target to reasonable
/// stopping position in cm from home
...
/// get_stopping_point_xy - calculates stopping point based on current position,
/// velocity, vehicle acceleration
///     distance_max allows limiting distance to stopping point
///     results placed in stopping_position vector
///     set_max_accel_xy() should be called before this method to set vehicle
///     acceleration set_leash_length() should have been called before this
///     method
void AC_PosControl::get_stopping_point_xy(Vector3f &stopping_point) const {
  const Vector3f curr_pos = _inav.get_position();
  Vector3f curr_vel = _inav.get_velocity();
  float linear_distance; // the distance at which we swap from a linear to sqrt
                         // response
  float linear_velocity; // the velocity above which we swap from a linear to
                         // sqrt response
  ...
  // avoid divide by zero by using current position if the velocity is below
  // 10cm/s, kP is very low or acceleration is zero
  ...
  if (vel_total < linear_velocity) {
    ...
    stopping_dist =
        linear_distance + (vel_total * vel_total) / (2.0f * _accel_cms);
  }
...
// is_active_xy - returns true if the xy position controller has been run very
// recently
bool AC_PosControl::is_active_xy() const {
  return ((AP_HAL::micros64() - _last_update_xy_us) <=
          POSCONTROL_ACTIVE_TIMEOUT_US);
}

/// get_lean_angle_max_cd - returns the maximum lean angle the autopilot may
/// request
...
///     this should be called after setting the position target and the desired
///     velocity and acceleration sets target roll angle, pitch angle and I
///     terms based on vehicle current lean angles should be called once
///     whenever significant changes to the position target are made this does
///     not update the xy target
void AC_PosControl::init_xy_controller() {
  // set roll, pitch lean angle targets to current attitude
  // todo: this should probably be based on the desired attitude not the current
  // attitude
...
/// update_xy_controller - run the horizontal position controller - should be
...
void AC_PosControl::write_log() {
  ...
  AP::logger().Write("PSC", "TimeUS,TPX,TPY,PX,PY,TVX,TVY,VX,VY,TAX,TAY,AX,AY",
                     "smmmmnnnnoooo", "F000000000000", "Qffffffffffff",
                     AP_HAL::micros64(), double(pos_target.x * 0.01f),
                     double(pos_target.y * 0.01f), double(position.x * 0.01f),
                     double(position.y * 0.01f), double(vel_target.x * 0.01f),
                     double(vel_target.y * 0.01f), double(velocity.x * 0.01f),
                     double(velocity.y * 0.01f), double(accel_target.x * 0.01f),
                     double(accel_target.y * 0.01f), double(accel_x * 0.01f),
                     double(accel_y * 0.01f));
}

/// init_vel_controller_xyz - initialise the velocity controller - should be
/// called once before the caller attempts to use the controller
...
/// update_velocity_controller_xy - run the velocity controller - should be
/// called at 100hz or higher
///     velocity targets should we set using set_desired_velocity_xy() method
///     callers should use get_roll() and get_pitch() methods and sent to the
///     attitude controller throttle targets will be sent directly to the motors
...
/// update_velocity_controller_xyz - run the velocity controller - should be
/// called at 100hz or higher
...
/// calc_leash_length - calculates the horizontal leash length given a maximum
/// speed, acceleration
///     should be called whenever the speed, acceleration or position kP is
///     modified
void AC_PosControl::calc_leash_length_xy() {
  // todo: remove _flags.recalc_leash_xy or don't call this function after each
  // variable change.
...
///     lat/lon frame converts desired accelerations provided in lat/lon frame
///     to roll/pitch angles
void AC_PosControl::run_xy_controller(float dt) {
  float ekfGndSpdLimit, ekfNavVelGainScaler;
  AP::ahrs_navekf().getEkfControlLimits(ekfGndSpdLimit, ekfNavVelGainScaler);

  Vector3f curr_pos = _inav.get_position();
  float kP = ekfNavVelGainScaler *
             _p_pos_xy.kP(); // scale gains to compensate for noisy optical flow
                             // measurement in the EKF
  ...
  if (kP <= 0.0f) {
    ...
    // Constrain the maximum length of _vel_target to the maximum position
    // correction velocity
    // TODO: replace the leash length with a user definable maximum position
    // correction
  ...
  // the following section converts desired velocities in lat/lon directions to
  // accelerations in lat/lon frame
  ...
  // update i term if we have not hit the accel or throttle limits OR the i term
  // will reduce
  ...
  // acceleration to correct for velocity error and scale PID output to
  // compensate for optical flow measurement induced EKF noise
  ...
  _accel_target_filter.set_cutoff_frequency(
      MIN(_accel_xy_filt_hz, 5.0f * ekfNavVelGainScaler));
  ...
  // the following section converts desired accelerations provided in lat/lon
  // frame to roll/pitch angles

  // limit acceleration using maximum lean angles
  float angle_max = MIN(_attitude_control.get_althold_lean_angle_max(),
                        get_lean_angle_max_cd());
  float accel_max = MIN(GRAVITY_MSS * 100.0f * tanf(ToRad(angle_max * 0.01f)),
                        POSCONTROL_ACCEL_XY_MAX);
  _limit.accel_xy =
      limit_vector_length(_accel_target.x, _accel_target.y, accel_max);

  // update angle targets that will be passed to stabilize controller
  accel_to_lean_angles(_accel_target.x, _accel_target.y, _roll_target,
                       _pitch_target);
}

// get_lean_angles_to_accel - convert roll, pitch lean angles to lat/lon frame
// accelerations in cm/s/s
void AC_PosControl::accel_to_lean_angles(float accel_x_cmss, float accel_y_cmss,
                                         float &roll_target,
                                         float &pitch_target) const {
  float accel_right, accel_forward;

  // rotate accelerations into body forward-right frame
  // todo: this should probably be based on the desired heading not the current
  // heading
  accel_forward =
      accel_x_cmss * _ahrs.cos_yaw() + accel_y_cmss * _ahrs.sin_yaw();
  accel_right =
      -accel_x_cmss * _ahrs.sin_yaw() + accel_y_cmss * _ahrs.cos_yaw();

  // update angle targets that will be passed to stabilize controller
  pitch_target =
      atanf(-accel_forward / (GRAVITY_MSS * 100.0f)) * (18000.0f / M_PI);
  float cos_pitch_target = cosf(pitch_target * M_PI / 18000.0f);
  roll_target = atanf(accel_right * cos_pitch_target / (GRAVITY_MSS * 100.0f)) *
                (18000.0f / M_PI);
}

// get_lean_angles_to_accel - convert roll, pitch lean angles to lat/lon frame
// accelerations in cm/s/s
void AC_PosControl::lean_angles_to_accel(float &accel_x_cmss,
                                         float &accel_y_cmss) const {
  // rotate our roll, pitch angles into lat/lon frame
  // todo: this should probably be based on the desired attitude not the current
  // attitude
  accel_x_cmss = (GRAVITY_MSS * 100) *
                 (-_ahrs.cos_yaw() * _ahrs.sin_pitch() * _ahrs.cos_roll() -
                  _ahrs.sin_yaw() * _ahrs.sin_roll()) /
                 MAX(_ahrs.cos_roll() * _ahrs.cos_pitch(), 0.5f);
  accel_y_cmss = (GRAVITY_MSS * 100) *
                 (-_ahrs.sin_yaw() * _ahrs.sin_pitch() * _ahrs.cos_roll() +
                  _ahrs.cos_yaw() * _ahrs.sin_roll()) /
                 MAX(_ahrs.cos_roll() * _ahrs.cos_pitch(), 0.5f);
}

/// calc_leash_length - calculates the horizontal leash length given a maximum
/// speed, acceleration and position kP gain
float AC_PosControl::calc_leash_length(float speed_cms, float accel_cms,
                                       float kP) const {
  float leash_length;
...
bool AC_PosControl::limit_vector_length(float &vector_x, float &vector_y,
                                        float max_length) {
...
/// Proportional controller with piecewise sqrt sections to constrain second
/// derivative
Vector3f AC_PosControl::sqrt_controller(const Vector3f &error, float p,
  ...
  if (error_length > linear_dist) {
    float first_order_scale = safe_sqrt(2.0f * second_ord_lim *
                                        (error_length - (linear_dist * 0.5f))) /
                              error_length;
    return Vector3f(error.x * first_order_scale, error.y * first_order_scale,
                    error.z);
  } else {
    return Vector3f(error.x * p, error.y * p, error.z);
  }
...
bool AC_PosControl::pre_arm_checks(const char *param_prefix, char *failure_msg,
  ...
  for (uint8_t i = 0; i < ARRAY_SIZE(ps); i++) {
    ...
    if (!is_positive(ps[i].p.kP())) {
      hal.util->snprintf(failure_msg, failure_msg_len, "%s_%s_P must be > 0",
                         param_prefix, ps[i].pid_name);
      return false;
    }
  }
  ...
  if (!is_positive(get_accel_z_pid().kP())) {
    hal.util->snprintf(failure_msg, failure_msg_len, "%s_ACCZ_P must be > 0",
                       param_prefix);
    return false;
  }
  if (!is_positive(get_accel_z_pid().kI())) {
    hal.util->snprintf(failure_msg, failure_msg_len, "%s_ACCZ_I must be > 0",
                       param_prefix);
    return false;
  }
