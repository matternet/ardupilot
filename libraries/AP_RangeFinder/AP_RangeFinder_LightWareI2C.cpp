...
static const char *parse_stream_id[NUM_SF20_DATA_STREAMS] =
    { // List of stream identifier strings. Must match init_stream_id[].
        STREAM1_VAL ":"};

static const char *init_stream_id[NUM_SF20_DATA_STREAMS] =
    { // List of stream initialization strings. Must match parse_stream_id[].
        "$1" STREAM1_VAL "\r\n"};

static const uint8_t streamSequence[] = {
    0}; // List of 0 based stream Ids that determine the LiDAR values collected.

static const uint8_t numStreamSequenceIndexes =
    sizeof(streamSequence) / sizeof(streamSequence[0]);

static uint32_t last_init_time_ms = 0; // Track timing before last init.

static const uint16_t MIN_WAIT_TIME_BEFORE_REINIT_MS =
    500; // Allow some time before reinits. Helps avoid spamming I2C bus.
static const uint16_t MIN_WAIT_TIME_BEFORE_VERSION_STRING_RETRY_MS =
    10; // Allow some time before retrying string to avoid spamming I2C bus.
        // Determined through general testing. Longer waits will not increase
        // performance and shorter waits are negligible on speed optimization.

static const uint8_t MAX_VERSION_STRING_RETRY_ATTEMPTS =
    4; // Attempts to get version string. Not blocking if these attempts fail.
       // 1 to set the Lidar into I2C mode. + 3 for general rety attempts = 4
static const uint8_t MAX_READ_ERRORS_THRESHOLD =
    5; // Max read errors allowed before a reinit attempt.
...
AP_RangeFinder_LightWareI2C::AP_RangeFinder_LightWareI2C(
    ...
    : AP_RangeFinder_Backend(_state, _params), version_("UNKNOWN"),
      _dev(std::move(dev)), read_errors_(0) {}
...
AP_RangeFinder_LightWareI2C::detect(RangeFinder::RangeFinder_State &_state,
 ...
 */
bool AP_RangeFinder_LightWareI2C::write_bytes(uint8_t *write_buf_u8,
                                              uint32_t len_u8) {
 ...
 */
bool AP_RangeFinder_LightWareI2C::sf20_send_and_expect(
    const char *send_msg, const char *expected_reply) {
 ...
 */
void AP_RangeFinder_LightWareI2C::get_version(char *buffer,
                                              uint8_t length) const {
  if (!buffer || length == 0) {
    return;
  }
  snprintf(buffer, length, "%s%s", RANGEFINDER_LIDAR_I2C_VERSION_PREFIX,
           version_);
}
...
/*
  ...
  version string
 */
bool AP_RangeFinder_LightWareI2C::sf20_get_version(const char *send_msg,
 ...
 * If for any reason this fails, the driver attempts to initialize the legacy
 * LightWare LiDAR. If this fails, the driver returns false indicating no
 * LightWare LiDAR is present.
 */
...
bool AP_RangeFinder_LightWareI2C::sf20_init() {
  ...
  // Attempt to get lidar version up to MAX_VERSION_STRING_RETRY_ATTEMPTS times.
  // This allows the UART to flush in between inits.
  ...
  // Turns off Address Tagging just in case it was previously left on in the
  ...
  // 0 = power off
  // 1 = power on
  // As of 7/10/17 sw and fw version 1.0 the "#E,1" command does not seem to be
  // supported. When it is supported the expected response would be "e:1".
...
bool AP_RangeFinder_LightWareI2C::sf20_get_reading(uint16_t &reading_cm) {
  ...
  // If a parse fails, the stream measurement is not updated until it is
  // successfully read in the future.
  uint8_t stream[lx20_max_expected_stream_reply_len_bytes +
                 1]; // Maximum response length for a stream ie "ldf,0:40.99" is
                     // 11 characters
  ...
  if (!sf20_parse_stream(stream, &num_processed_chars, parse_stream_id[i],
                         sf20_stream_val[i])) {
    ++read_errors_;
    return false;
  }
...
bool AP_RangeFinder_LightWareI2C::sf20_parse_stream(
    uint8_t *stream_buf, size_t *p_num_processed_chars,
    const char *string_identifier, uint16_t &val) {
   ...
   */
  if (strncmp((const char *)&stream_buf[*p_num_processed_chars], "-1.00", 5) ==
      0) {
  ...
  /* Number is always returned in hundredths. So 6.33 is returned as 633. 6.3 is
   * returned as 630. 6 is returned as 600. Extract number in format 6.33 or
   * 123.99 (meters to be converted to centimeters). Percentages such as 100
   * (percent), are returned as 10000.
   */
  ...
  while ((((digit_u16 <= '9') && (digit_u16 >= '0')) || (digit_u16 == '.')) &&
  ...
  if (read_errors_ < MAX_READ_ERRORS_THRESHOLD) {
    if (sf20_get_reading(state.distance_cm)) {
      ...
      if (count % TIMER_FREQ_HZ == 0) {
        gcs().send_text(MAV_SEVERITY_INFO, "RANGEFINDER: %s%s",
                        RANGEFINDER_LIDAR_I2C_VERSION_PREFIX, version_);
...
// Only for use during init as this blocks while waiting for the SF20 to be
// ready.
bool AP_RangeFinder_LightWareI2C::sf20_wait_on_reply(uint8_t *rx_two_byte) {
  ...
  // This is used after a read command to allow the sf20 time to provide the
  // result.
