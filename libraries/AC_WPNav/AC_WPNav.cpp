...
const AP_Param::GroupInfo AC_WPNav::var_info[] = {
    ...
    // @Description: Defines the speed in cm/s which the aircraft will attempt
    // to maintain horizontally during a WP mission
    // @Units: cm/s
    // @Range: 20 2000
    // @Increment: 50
    // @User: Standard
    ...
    // @Description: Defines the distance from a waypoint, that when crossed
    ...
    // @Description: Defines the speed in cm/s which the aircraft will attempt
    // to maintain while climbing during a WP mission
    // @Units: cm/s
    // @Range: 10 1000
    // @Increment: 50
    // @User: Standard
    ...
    // @Description: Defines the speed in cm/s which the aircraft will attempt
    // to maintain while descending during a WP mission
    // @Units: cm/s
    // @Range: 10 500
    // @Increment: 10
    // @User: Standard
    ...
    // @Description: Defines the horizontal acceleration in cm/s/s used during
    // missions
    // @Units: cm/s/s
    // @Range: 50 500
    // @Increment: 10
    ...
    // @Description: Defines the vertical acceleration in cm/s/s used during
    // missions
    // @Units: cm/s/s
    // @Range: 50 500
    // @Increment: 10
    ...
    // @Description: This controls if waypoint missions use rangefinder for
    // terrain following
    ...
    // @Description: This controls the maximum crosstrack distance in waypoint
    // missions for fast waypoints. A value of zero means no limit
    // @Range: 0 200
    // @User: Advanced
    // @Units: m
...
AC_WPNav::AC_WPNav(const AP_InertialNav &inav, const AP_AHRS_View &ahrs,
    ...
    : _inav(inav), _ahrs(ahrs), _pos_control(pos_control),
...
/// wp_and_spline_init - initialise straight line and spline waypoint
/// controllers
///     updates target roll, pitch targets and I terms based on vehicle lean
///     angles should be called once before the waypoint controller is used but
...
/// set_speed_xy - allows main code to pass target horizontal velocity for wp
/// navigation
...
///     returns false if conversion from location to vector from ekf origin
...
///     terrain_alt should be true if destination.z is a desired altitude above
///     terrain
bool AC_WPNav::set_wp_destination(const Vector3f &destination,
  ...
  if ((AP_HAL::millis() - _wp_last_update) < 1000) {
    ...
    // if waypoint controller is not active, set origin to reasonable stopping
...
bool AC_WPNav::set_wp_destination_NED(const Vector3f &destination_NED) {
  ...
  return set_wp_destination(Vector3f(destination_NED.x * 100.0f,
                                     destination_NED.y * 100.0f,
                                     -destination_NED.z * 100.0f),
                            false);
}

/// set_origin_and_destination - set origin and destination waypoints using
/// position vectors (distance from home in cm)
///     terrain_alt should be true if origin.z and destination.z are desired
///     altitudes above terrain (false if these are alt-above-ekf-origin)
...
bool AC_WPNav::set_wp_origin_and_destination(const Vector3f &origin,
  ...
  _track_length_xy = safe_sqrt(
      sq(pos_delta.x) + sq(pos_delta.y)); // get horizontal track length (used
                                          // to decide if we should update yaw)
  ...
  _flags.segment_type = SEGMENT_STRAIGHT;
  _flags.new_wp_destination =
      true; // flag new waypoint so we can freeze the pos controller's feed
            // forward and smooth the transition
  _flags.wp_yaw_set = false;

  // initialise the limited speed to current speed along the track
  const Vector3f &curr_vel = _inav.get_velocity();
  // get speed along track (note: we convert vertical speed into horizontal
  // speed equivalent)
  ...
  _limited_speed_xy_cms =
      constrain_float(speed_along_track, 0, _pos_control.get_max_speed_xy());

  return true;
}

/// shift_wp_origin_to_current_pos - shifts the origin and destination so the
/// origin starts at the current position
///     used to reset the position just before takeoff
///     relies on set_wp_destination or set_wp_origin_and_destination having
///     been called first
...
/// get_wp_stopping_point_xy - returns vector to stopping point based on a
/// horizontal position and velocity
void AC_WPNav::get_wp_stopping_point_xy(Vector3f &stopping_point) const {
  _pos_control.get_stopping_point_xy(stopping_point);
}

/// get_wp_stopping_point - returns vector to stopping point based on 3D
...
/// advance_wp_target_along_track - move target location along track from origin
/// to destination
bool AC_WPNav::advance_wp_target_along_track(float dt) {
  float track_covered;  // distance (in cm) along the track that the vehicle has
                        // traveled.  Measured by drawing a perpendicular line
                        // from the track to the vehicle.
  Vector3f track_error; // distance error (in cm) from the track_covered
                        // position (i.e. closest point on the line to the
                        // vehicle) and the vehicle
  float track_desired_max; // the farthest distance (in cm) along the track that
                           // the leash will allow
  float
      track_leash_slack; // additional distance (in cm) along the track from our
                         // track_covered position that our leash will allow
  bool reached_leash_limit = false; // true when track has reached leash limit
                                    // and we need to slow down the target point
  ...
  track_covered = curr_delta.x * _pos_delta_unit.x +
                  curr_delta.y * _pos_delta_unit.y +
                  curr_delta.z * _pos_delta_unit.z;

  // calculate the point closest to the vehicle on the segment from origin to
  // destination
  Vector3f track_covered_pos = _pos_delta_unit * track_covered;

  // calculate the distance vector from the vehicle to the closest point on the
  // segment from origin to destination
  ...
  float leash_z = track_error.z >= 0 ? _pos_control.get_leash_up_z()
                                     : _pos_control.get_leash_down_z();

  // use pythagoras's theorem calculate how far along the track we could move
  // the intermediate target before reaching the end of the leash
  //   track_desired_max is the distance from the vehicle to our target point
  //   along the track.  It is the "hypotenuse" which we want to be no longer
  //   than our leash (aka _track_leash_length) track_error is the line from the
  //   vehicle to the closest point on the track.  It is the "opposite" side
  //   track_leash_slack is the line from the closest point on the track to the
  //   target point.  It is the "adjacent" side.  We adjust this so the
  //   track_desired_max is no longer than the leash
  float track_leash_length_abs = fabsf(_track_leash_length);
  float track_error_max_abs =
      MAX(_track_leash_length * track_error_z / leash_z,
          _track_leash_length * _track_error_xy / _pos_control.get_leash_xy());
  track_leash_slack =
      (track_leash_length_abs > track_error_max_abs)
          ? safe_sqrt(sq(_track_leash_length) - sq(track_error_max_abs))
          : 0;
  ...
  // let the limited_speed_xy_cms be some range above or below current velocity
  // along track
  if (speed_along_track < -linear_velocity) {
    // we are traveling fast in the opposite direction of travel to the waypoint
    // so do not move the intermediate point
    _limited_speed_xy_cms = 0;
  } else {
    // increase intermediate target point's velocity if not yet at the leash
    // limit
    ...
    _limited_speed_xy_cms =
        constrain_float(_limited_speed_xy_cms, 0.0f, _track_speed);
    ...
    if (!_flags.fast_waypoint) {
      ...
      if (_flags.slowing_down) {
        _limited_speed_xy_cms =
            MIN(_limited_speed_xy_cms,
                get_slow_down_speed(dist_to_dest, _track_accel));
      }
    }

    // if our current velocity is within the linear velocity range limit the
    // intermediate point's velocity to be no more than the linear_velocity
    // above or below our current velocity
    if (fabsf(speed_along_track) < linear_velocity) {
      _limited_speed_xy_cms = constrain_float(
          _limited_speed_xy_cms, speed_along_track - linear_velocity,
  ...
  // do not let desired point go past the end of the track unless it's a fast
  // waypoint
  if (!_flags.fast_waypoint) {
    _track_desired = constrain_float(_track_desired, 0, _track_length);
  } else {
    _track_desired = constrain_float(
        _track_desired, 0, _track_length + WPNAV_WP_FAST_OVERSHOOT_MAX);
  }
  ...
  _pos_control.set_pos_target(0);

  // check if we've reached the waypoint
  if (!_flags.reached_destination) {
    if (_track_desired >= _track_length) {
      // "fast" waypoints are complete once the intermediate point reaches the
      // destination
      if (_flags.fast_waypoint) {
        _flags.reached_destination = true;
      } else {
        // regular waypoints also require the copter to be within the waypoint
        // radius
        Vector3f dist_to_dest =
            (curr_pos - Vector3f(0, 0, terr_offset)) - _destination;
  ...
  // update the target yaw if origin and destination are at least 2m apart
  // horizontally
  if (_track_length_xy >= WPNAV_YAW_DIST_MIN) {
    if (_pos_control.get_leash_xy() < WPNAV_YAW_DIST_MIN) {
      // if the leash is short (i.e. moving slowly) and destination is at least
      // 2m horizontally, point along the segment from origin to destination
      ...
      if (horiz_leash_xy.length() >
        ...
        set_yaw_cd(
            RadiansToCentiDegrees(atan2f(horiz_leash_xy.y, horiz_leash_xy.x)));
      }
    }
  }
...
/// get_wp_distance_to_destination - get horizontal distance to destination in
/// cm
...
/// get_wp_bearing_to_destination - get bearing to next waypoint in
/// centi-degrees
...
bool AC_WPNav::update_wpnav() {
  ...
  // wp_speed_update - update _pos_control.set_max_speed_xy if speed change has
  // been requested
  wp_speed_update(dt);

  // advance the target if necessary
  if (!advance_wp_target_along_track(dt)) {
    // To-Do: handle inability to advance along track (probably because of
...
// check_wp_leash_length - check if waypoint leash lengths need to be
// recalculated
//  should be called after _pos_control.update_xy_controller which may have
//  changed the position controller leash lengths
...
/// calculate_wp_leash_length - calculates horizontal and vertical leash lengths
/// for waypoint controller
void AC_WPNav::calculate_wp_leash_length() {
  ...
  // calculate the maximum acceleration, maximum velocity, and leash length in
  ...
  if (is_zero(pos_delta_unit_z) && is_zero(pos_delta_unit_xy)) {
    ...
    _track_speed = MIN(speed_z / pos_delta_unit_z,
                       _pos_control.get_max_speed_xy() / pos_delta_unit_xy);
    _track_leash_length = MIN(leash_z / pos_delta_unit_z,
                              _pos_control.get_leash_xy() / pos_delta_unit_xy);
  }

  // calculate slow down distance (the distance from the destination when the
  // target point should begin to slow down)
...
///     returns false if conversion from location to vector from ekf origin
///     cannot be calculated stopped_at_start should be set to true if vehicle
///     is stopped at the origin seg_end_type should be set to stopped, straight
///     or spline depending upon the next segment's type next_destination should
///     be set to the next segment's destination if the seg_end_type is
///     SEGMENT_END_STRAIGHT or SEGMENT_END_SPLINE
bool AC_WPNav::set_spline_destination(const Location &destination,
...
/// set_spline_destination waypoint using position vector (distance from home in
/// cm)
///     returns false if conversion from location to vector from ekf origin
///     cannot be calculated terrain_alt should be true if destination.z is a
///     desired altitudes above terrain (false if its desired altitudes above
///     ekf origin) stopped_at_start should be set to true if vehicle is stopped
///     at the origin seg_end_type should be set to stopped, straight or spline
///     depending upon the next segment's type next_destination should be set to
///     the next segment's destination if the seg_end_type is
///     SEGMENT_END_STRAIGHT or SEGMENT_END_SPLINE
bool AC_WPNav::set_spline_destination(const Vector3f &destination,
  ...
  // if waypoint controller is active and copter has reached the previous
  // waypoint use current pos target as the origin
  if ((AP_HAL::millis() - _wp_last_update) < 1000) {
    origin = _pos_control.get_pos_target();
  } else {
  ...
  return set_spline_origin_and_destination(origin, destination, terrain_alt,
                                           stopped_at_start, seg_end_type,
                                           next_destination);
}

/// set_spline_origin_and_destination - set origin and destination waypoints
/// using position vectors (distance from home in cm)
///     terrain_alt should be true if origin.z and destination.z are desired
///     altitudes above terrain (false if desired altitudes above ekf origin)
///     seg_type should be calculated by calling function based on the mission
bool AC_WPNav::set_spline_origin_and_destination(
    const Vector3f &origin, const Vector3f &destination, bool terrain_alt,
    bool stopped_at_start, spline_segment_end_type seg_end_type,
    const Vector3f &next_destination) {
  // mission is "active" if wpnav has been called recently and vehicle reached
  ...
  // straight-fast - vehicle is moving, previous segment is straight.  vehicle
  // will fly straight through the waypoint before beginning it's spline path to
  // the next wp
  //     _flag.segment_type holds whether prev segment is straight vs spline but
  //     we don't know if it has a delay
  // spline-fast - vehicle is moving, previous segment is splined, vehicle will
  // fly through waypoint but previous segment should have it flying in the
  // correct direction (i.e. exactly parallel to position difference vector from
  // previous segment's origin to this segment's destination)

  // calculate spline velocity at origin
  if (stopped_at_start || !prev_segment_exists) {
    // if vehicle is stopped at the origin, set origin velocity to 0.02 *
    // distance vector from origin to destination
    ...
    if (_flags.segment_type == SEGMENT_STRAIGHT) {
      // previous segment is straight, vehicle is moving so vehicle should fly
      // straight through the origin before beginning it's spline path to the
      // next waypoint. Note: we are using the previous segment's origin and
      // destination
      _spline_origin_vel = (_destination - _origin);
      _spline_time = 0.0f; // To-Do: this should be set based on how much
                           // overrun there was from straight segment?
      _spline_vel_scaler =
          _pos_control.get_vel_target()
              .length(); // start velocity target from current target velocity
    } else {
      // previous segment is splined, vehicle will fly through origin
      // we can use the previous segment's destination velocity as this
      // segment's origin velocity Note: previous segment will leave destination
      // velocity parallel to position difference vector
      ...
      if (_spline_time > 1.0f &&
          _spline_time < 1.1f) { // To-Do: remove hard coded 1.1f
        _spline_time -= 1.0f;
      } else {
        _spline_time = 0.0f;
      }
      // Note: we leave _spline_vel_scaler as it was from end of previous
      // segment
    }
  }
  ...
  case SEGMENT_END_STOP:
    // if vehicle stops at the destination set destination velocity to 0.02 *
    // distance vector from origin to destination
  ...
  case SEGMENT_END_STRAIGHT:
    // if next segment is straight, vehicle's final velocity should face along
    // the next segment's position
    _spline_destination_vel = (next_destination - destination);
    _flags.fast_waypoint = true;
    break;

  case SEGMENT_END_SPLINE:
    // if next segment is splined, vehicle's final velocity should face parallel
    // to the line from the origin to the next destination
    _spline_destination_vel = (next_destination - origin);
    _flags.fast_waypoint = true;
    break;
  }

  // code below ensures we don't get too much overshoot when the next segment is
  // short
  float vel_len =
      _spline_origin_vel.length() + _spline_destination_vel.length();
  float pos_len = (destination - origin).length() * 4.0f;
  if (vel_len > pos_len) {
    // if total start+stop velocity is more than twice position difference
    // use a scaled down start and stop velocityscale the  start and stop
    // velocities down
  ...
  _track_length_xy = safe_sqrt(
      sq(_destination.x - _origin.x) +
      sq(_destination.y - _origin.y)); // horizontal track length (used to
                                       // decide if we should update yaw)
...
bool AC_WPNav::update_spline() {
  ...
  if (!advance_spline_target_along_track(dt)) {
    // To-Do: handle failure to advance along track (due to missing terrain
    // data)
    ret = false;
  }
...
///		relies on _spline_origin_vel, _spline_destination_vel and
///_origin and _destination
void AC_WPNav::update_spline_solution(const Vector3f &origin,
  ...
  _hermite_spline_solution[2] =
      -origin * 3.0f - origin_vel * 2.0f + dest * 3.0f - dest_vel;
  _hermite_spline_solution[3] =
      origin * 2.0f + origin_vel - dest * 2.0f + dest_vel;
}

/// advance_spline_target_along_track - move target location along track from
...
bool AC_WPNav::advance_spline_target_along_track(float dt) {
  if (!_flags.reached_destination) {
    ...
    // get terrain altitude offset for origin and current position (i.e. change
    // in terrain altitude from a position vs ekf origin)
    float terr_offset = 0.0f;
    if (_commanded_alt_enabled || !is_zero(_commanded_alt_offset_cm)) {
      terr_offset = commanded_alt_target_offset(dt);
    } else if (_terrain_alt && !get_terrain_offset(terr_offset)) {
    ...
    // calculate how far along the track we could move the intermediate target
    // before reaching the end of the leash
    float track_leash_slack =
        MIN(_track_leash_length * (leash_z - track_error_z) / leash_z,
    ...
    // if within the stopping distance from destination, set target velocity to
    // sqrt of distance * 2 * acceleration
    ...
    // scale the spline_time by the velocity we've calculated vs the velocity
    // that came out of the spline calculator
    ...
    if (_track_length_xy >= WPNAV_YAW_DIST_MIN) {
      if (_pos_control.get_leash_xy() < WPNAV_YAW_DIST_MIN) {
        // if the leash is very short (i.e. flying at low speed) use the target
        // point's velocity along the track
        if (!is_zero(target_vel.x) && !is_zero(target_vel.y)) {
          set_yaw_cd(RadiansToCentiDegrees(atan2f(target_vel.y, target_vel.x)));
        }
      } else {
        // point vehicle along the leash (i.e. point vehicle towards target
        // point on the segment from origin to destination)
        float track_error_xy_length =
            safe_sqrt(sq(track_error.x) + sq(track_error.y));
        if (track_error_xy_length >
            MIN(WPNAV_YAW_DIST_MIN,
                _pos_control.get_leash_xy() * WPNAV_YAW_LEASH_PCT_MIN)) {
          // To-Do: why is track_error sign reversed?
          set_yaw_cd(
              RadiansToCentiDegrees(atan2f(-track_error.y, -track_error.x)));
        }
      }
    }
    ...
    // we will reach the next waypoint in the next step so set
    // reached_destination flag To-Do: is this one step too early?
    if (_spline_time >= 1.0f) {
      _flags.reached_destination = true;
    }
  }
...
// calc_spline_pos_vel_accel - calculates target position, velocity and
// acceleration for the given "spline_time"
/// 	relies on update_spline_solution being called when the segment's origin
/// and destination were set
...
// get terrain's altitude (in cm above the ekf origin) at the current position
// (+ve means terrain below vehicle is above ekf origin's altitude)
...
// convert location to vector from ekf origin.  terrain_alt is set to true if
// resulting vector's z-axis should be treated as alt-above-terrain
//      returns false if conversion failed (likely because terrain data was not
//      available)
bool AC_WPNav::get_vector_NEU(const Location &loc, Vector3f &vec,
                              bool &terrain_alt) {
  ...
  // copy xy (we do this to ensure we do not adjust vector unless the overall
...
/// calc_slow_down_distance - calculates distance before waypoint that target
/// point should begin to slow-down assuming it is travelling at full speed
...
/// get_slow_down_speed - returns target speed of target point based on distance
/// from the destination (in cm)
...
float AC_WPNav::commanded_alt_target_offset(float dt) {
  if (_commanded_alt_enabled) {
    _commanded_alt_offset_cm = constrain_float(
        _commanded_alt_offset_cm + _commanded_alt_cm -
            _pos_control.get_alt_target(),
        _commanded_alt_offset_cm - dt * fabsf(_wp_speed_down_cms),
        _commanded_alt_offset_cm + dt * _wp_speed_up_cms);
    if (AP_HAL::millis() - _commanded_alt_last_update_ms >
        WPNAV_COMMANDED_ALT_TIMEOUT_MS) {
      _commanded_alt_enabled = false;
    }
  } else {
    _commanded_alt_offset_cm = constrain_float(
        0.0, _commanded_alt_offset_cm - dt * fabsf(_wp_speed_down_cms),
        _commanded_alt_offset_cm + dt * _wp_speed_up_cms);
  }
...
void AC_WPNav::set_pause(bool paused) { _mission_paused = paused; }
