...
void Copter::arm_motors_check() {
  ...
  if (yaw_in > 4000) {
    ...
    // increase the arming counter to a maximum of 1 beyond the auto trim
    // counter
    ...
    // full left and rudder disarming is enabled
             ...
             (arming_rudder == AP_Arming::RudderArming::ARMDISARM)) {
...
// auto_disarm_check - disarms the copter if it has been sitting on the ground
// in manual mode with throttle low for at least 15 seconds
void Copter::auto_disarm_check() {
  ...
  if (!motors->armed() || disarm_delay_ms == 0 ||
      control_mode == Mode::Number::THROW) {
  ...
  // always allow auto disarm if using interlock switch or motors are Emergency
  // Stopped
  if ((ap.using_interlock && !motors->get_interlock()) ||
      SRV_Channels::get_emergency_stop()) {
#if FRAME_CONFIG != HELI_FRAME
    // use a shorter delay if using throttle interlock switch or Emergency Stop,
    // because it is less obvious the copter is armed as the motors will not be
    // spinning
    disarm_delay_ms /= 2;
#endif
  } else {
...
// motors_output - send output to motors library which will adjust and send to
// ESCs and servos
void Copter::motors_output() {
  ...
  if (ap.in_arming_delay &&
      (!motors->armed() || millis() - arm_time_ms > ARMING_DELAY_SEC * 1.0e3f ||
       control_mode == Mode::Number::THROW)) {
  ...
  if (ap.motor_test) {
    ...
    bool interlock = motors->armed() && !ap.in_arming_delay &&
...
void Copter::lost_vehicle_check() {
  ...
  // ensure throttle is down, motors not armed, pitch and roll rc at max. Note:
  // rc1=roll rc2=pitch
  if (ap.throttle_zero && !motors->armed() &&
      (channel_roll->get_control_in() > 4000) &&
      (channel_pitch->get_control_in() > 4000)) {
